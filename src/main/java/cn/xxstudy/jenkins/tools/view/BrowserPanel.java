...
import cn.xxstudy.jenkins.tools.util.ListUtils;
import cn.xxstudy.jenkins.tools.util.LogUtils;
...
import cn.xxstudy.jenkins.tools.*;
import cn.xxstudy.jenkins.tools.logic.*;
import cn.xxstudy.jenkins.tools.model.*;
import cn.xxstudy.jenkins.tools.util.CollectionUtil;
import cn.xxstudy.jenkins.tools.util.GuiUtil;
import cn.xxstudy.jenkins.tools.view.action.*;
import cn.xxstudy.jenkins.tools.view.action.settings.SortByStatusAction;
...
@State(name = "JenkinsBrowserPanel", storages = {@Storage(value = StoragePathMacros.PRODUCT_WORKSPACE_FILE, roamingType = RoamingType.DISABLED)})
public final class BrowserPanel extends SimpleToolWindowPanel implements PersistentStateComponent<JenkinsTreeState> {
    ...
    public Build getSelectedBuild() {
        return jobTree.getLastSelectedPath(JenkinsTreeNode.BuildNode.class).map(JenkinsTreeNode.BuildNode::getBuild).orElse(null);
    }

    @Nullable
    public Job getSelectedJob() {
        return jobTree.getLastSelectedPath(JenkinsTreeNode.JobNode.class).map(JenkinsTreeNode.JobNode::getJob).orElse(null);
    }

    public List<Job> getAllSelectedJobs() {
        return TreeUtil.collectSelectedObjectsOfType(jobTree.getTree(), JenkinsTreeNode.JobNode.class).stream().map(JenkinsTreeNode.JobNode::getJob).collect(Collectors.toList());
    }
    ...
    public void loadJob(final Job job, Consumer<Job> loadedJob) {
        ...
        JenkinsBackgroundTaskFactory.getInstance(project).createBackgroundTask("Loading job", true, new JenkinsBackgroundTask.JenkinsTask() {
    ...
    public void addToWatch(String changeListName, Job job) {
        ...
        if (lastBuild != null) {
            ...
            final Build nextBuild = lastBuild.toBuilder().number(nextBuildNumber).url(String.format("%s/%d/", job.getUrl(), nextBuildNumber)).build();
    ...
    private void watch() {
        ...
        if (!watchedJobs.isEmpty()) {
            for (final Map.Entry<String, Job> entry : watchedJobs.entrySet()) {
                ...
                final Build lastBuild = job.getLastBuild();
                JenkinsBackgroundTaskFactory.getInstance(project).createBackgroundTask("Jenkins build watch", true, new JenkinsBackgroundTask.JenkinsTask() {
                    ...
                    public void onSuccess() {
                        ...
                        if (lastBuild.isBuilding() && !build.isBuilding()) {
                            notifyInfoJenkinsToolWindow(String.format("Status of build for Changelist \"%s\" is %s", entry.getKey(), build.getStatus().getStatus()));
                        }
                    ...
                    public void onThrowable(@NotNull Throwable error) {
                        ...
                        notifyErrorJenkinsToolWindow(String.format("Error while watch for Changelist \"%s\" is %s", entry.getKey(), error.getMessage()));
                    }
                }).queue();
            }
        }
    ...
    public void expandSelectedJob() {
        Optional.ofNullable(jobTree.getLastSelectedPathComponent()).filter(node -> node.getUserObject() instanceof JenkinsTreeNode.JobNode).ifPresent(node -> jobTree.getTree().expandPath(new TreePath(node.getPath())));
    }
    ...
    private class LoadSelectedViewJob implements JenkinsBackgroundTask.JenkinsTask {
        ...
        public LoadSelectedViewJob(@NotNull Project project) {
            this.task = JenkinsBackgroundTaskFactory.getInstance(project).createBackgroundTask("Loading Jenkins Jobs", true, LoadSelectedViewJob.this);
        }
